---
name: gate_coverage
description: 产出文件门禁 (硬判定) — 脚本门禁 review skill：运行 check_coverage.cjs 校验产物，并按 OK/FAIL/BLOCKED 三态回报框架。
---

# 产出文件门禁 (硬判定)

<!-- gate-wrapper: shared-scripts/check_coverage.cjs -->
## 门禁校验（本节点唯一职责）
1. 运行校验脚本：`node {{SCRIPTS}}/check_coverage.cjs`。
